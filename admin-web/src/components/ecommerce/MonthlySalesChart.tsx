...
export default function MonthlySalesChart({ revenueChart, loading }: MonthlySalesChartProps) {
  const categories = revenueChart.map((p) => p.month);
  const seriesData = revenueChart.map((p) => p.revenue);
  ...
  const series = [{ name: "Doanh thu", data: seriesData }];
