...
const movieMutationBodySchema = strictObject({
  ...
  genres: stringListSchema.label('genres'),
  posterUrl: Joi.string().trim().allow('').max(1000).default('').label('posterUrl'),
  backdropUrl: Joi.string().trim().allow('').max(1000).default('').label('backdropUrl'),
  trailerUrl: Joi.string().trim().allow('').max(1000).default('').label('trailerUrl'),
  trailerProvider: Joi.string().trim().valid('youtube', 'vimeo', 'other').default('youtube').label('trailerProvider'),
